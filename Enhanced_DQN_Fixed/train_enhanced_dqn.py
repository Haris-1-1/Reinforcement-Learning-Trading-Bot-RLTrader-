"""
ENHANCED DQN TRADING BOT - MAIN TRAINING SCRIPT
================================================
Complete training pipeline with benchmarks and evaluation
"""
...
def run_benchmarks(prices: np.ndarray, initial_cash: float) -> dict:
    ...
    # 2. Random Trading (average of 10 runs)
    random_returns = []
...
def train():
    ...
    for episode in range(1, CONFIG['agent']['episodes'] + 1):
        ...
        while not done:
            # 1. Select action (with action masking)
            ...
            # 3. Store experience (with next action mask for Double DQN)
    ...
    # Run test
    agent.is_eval = True  # Disable exploration
